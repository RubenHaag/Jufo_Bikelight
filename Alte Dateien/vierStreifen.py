...
def bildAuslesen():
    ...
    for i in range(0, LED_COUNT):
        ...
        matrix[1][i] = int(round(cos(k1) * matrix[0][i], 0) + LED_COUNT)    #Berechnung der X-Koordinate
        matrix[2][i] = int(round(sin(k1) * matrix[0][i], 0) + LED_COUNT)
        r, g, b, _ = pix[matrix[1][i], matrix[2][i]]                        #auslesen eines Pixels
        streifen1.setPixelColor(i+1, Color(r, g, b))

        matrix[1][i] = int(round(cos(k2) * matrix[0][i], 0) + LED_COUNT)    #Berechnung der X-Koordinate
        matrix[2][i] = int(round(sin(k2) * matrix[0][i], 0) + LED_COUNT)
        r, g, b, _ = pix[matrix[1][i], matrix[2][i]]                        #auslesen eines Pixels
        streifen2.setPixelColor(i+1, Color(r, g, b))

        matrix[1][i] = int(round(cos(k3) * matrix[0][i], 0) + LED_COUNT)    #Berechnung der X-Koordinate
        matrix[2][i] = int(round(sin(k3) * matrix[0][i], 0) + LED_COUNT)
        r, g, b, _ = pix[matrix[1][i], matrix[2][i]]                        #auslesen eines Pixels
        streifen3.setPixelColor(i+1, Color(r, g, b))
...
def main():
    ...
    streifen1.begin()  #starten des LED-Streifens
    streifen2.begin()  #starten des LED-Streifens
    streifen3.begin()  #starten des LED-Streifens
    streifen4.begin()  #starten des LED-Streifens
...
streifen1 = Adafruit_NeoPixel(LED_COUNT, LED_PIN, LED_FREQ_HZ, LED_DMA, LED_INVERT, LED_BRIGHTNESS)
streifen2 = Adafruit_NeoPixel(LED_COUNT, LED_PIN, LED_FREQ_HZ, LED_DMA, LED_INVERT, LED_BRIGHTNESS)
streifen3 = Adafruit_NeoPixel(LED_COUNT, LED_PIN, LED_FREQ_HZ, LED_DMA, LED_INVERT, LED_BRIGHTNESS)
streifen4 = Adafruit_NeoPixel(LED_COUNT, LED_PIN, LED_FREQ_HZ, LED_DMA, LED_INVERT, LED_BRIGHTNESS)
